...
function createShade(){return $('<div><!-- // --></div>').css({position:'absolute',backgroundColor:options.bgColor}).appendTo(holder);}
function enableShade(){if(!enabled){enabled=true;holder.insertBefore($img);updateAuto();Selection.setBgOpacity(1,true);$img2.hide();if(Selection.isAwake())
setOpacity(options.bgOpacity);else setOpacity(1);}}
function setBgColor(){}
function disableShade(){if(enabled){holder.remove();enabled=false;$img2.show();Selection.setBgOpacity(options.bgOpacity);}}
function setOpacity(opacity){if(enabled){if(options.bgFade){holder.animate({opacity:1-opacity},{queue:false,duration:options.fadeTime});}
else holder.css({opacity:1-opacity});}}
function refreshAll(){options.shade?enableShade():disableShade();}
function getShades(){return holder.children();}
return{update:updateAuto,updateRaw:updateShade,getShades:getShades,enable:enableShade,disable:disableShade,resize:resizeShades,refresh:refreshAll,opacity:setOpacity};}());var Selection=(function(){var awake,hdep=370;var borders={};var handle={};var seehandles=false;var hhs=options.handleOffset;function insertBorder(type)
...
{if(!awake&&!force)return;if(options.bgFade){$img.animate({opacity:opacity},{queue:false,duration:options.fadeTime});}else{$img.css('opacity',opacity);}}
function show()
...
function colorChangeMacro($obj){if(supportsColorFade()&&options.fadeTime){$obj.animate({backgroundColor:options.bgColor},{queue:false,duration:options.fadeTime});}else{$obj.css('backgroundColor',options.bgColor);}}
function interfaceUpdate(alt)
{if(options.allowResize){if(alt){Selection.enableOnly();}else{Selection.enableHandles();}}else{Selection.disableHandles();}
Tracker.setCursor(options.allowSelect?'crosshair':'default');Selection.setCursor(options.allowMove?'move':'default');if(options.hasOwnProperty('setSelect')){setSelect(options.setSelect);Selection.done();delete(options.setSelect);}
if(options.hasOwnProperty('trueSize')){xscale=options.trueSize[0]/boundx;yscale=options.trueSize[1]/boundy;}
Shade.refresh();if(options.bgColor!=bgcolor){colorChangeMacro(options.shade?Shade.getShades():$div);bgcolor=options.bgColor;}
if(bgopacity!==options.bgOpacity){bgopacity=options.bgOpacity;if(Selection.isAwake()){if(options.shade)Shade.refresh();else Selection.setBgOpacity(bgopacity);}}
